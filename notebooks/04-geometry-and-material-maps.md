...
A 1.0 m wide, 0.6 m tall block whose lower-left corner sits at $(-0.5, -0.4)$, so it straddles the origin:
...
The overlap here is a circular segment: the disk cap above $y = 0.2$ has analytic area $0.25\cos^{-1}(0.4) - 0.2\sqrt{0.21} = 0.1982$, which is `d_only`; `both` is the rest of the disk ($0.7854 - 0.1982 = 0.5872$) and `r_only` is the rectangle minus that ($0.96 - 0.5872 = 0.3728$). All three derived regions rasterize with far larger relative error than a standalone shape (the full disk's staircase error was ≈ 0.2%): here `r_only` errs +2.0%, the sliver `d_only` +1.6%, and the overlap −0.7%. Composition is the amplifier — each region inherits rim error from every curved or off-grid boundary that cuts it, the signs need not cancel the way they largely do around a full closed rim, and smaller region areas inflate the relative figure.
...
2. **Pac-Man via subtraction.** Take a unit disk centred at the origin and remove a triangular "mouth" using `polygon_mask`. Verify that the resulting area is the disk area minus the area of the disk–triangle *overlap*, up to staircase error — a typical mouth triangle pokes past the rim, so subtracting the full triangle area over-corrects. (Or keep the triangle entirely inside the disk, where the two statements coincide.)
