...
> Looking for the actual rustlab_em curriculum? Jump straight to
> [`../book/`](../book/) — this doc is about the *layout pattern*, not
...
- Downstream course projects (this `rustlab_em` curriculum among them)
  commit their rendered output to `book/` — the name reads as "the
  rendered curriculum" to a first-time visitor and matches the
  conventions of mdBook and Jupyter Book.
- Upstream rustlab itself uses the same pattern at a larger scale
  (sources flat in `examples/notebooks/`, rendered output committed to
  a top-level dir called `gallery/`, no `lessons/` layer for side
  scripts) — `gallery/` reads correctly there because the entries are
  independent demos rather than a linear curriculum:
